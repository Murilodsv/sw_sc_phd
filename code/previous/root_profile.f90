subroutine root_profile(nlay			            , & ! Input
    ...
    do sl = 1, nsublay
        if(sl .le. aint(initcropdepth))then
            ...
            !--- Above planting depth            
            if(flemerged)then
                
                !--- All sub compartments are filled
                rootprof(sl) = 1.d0
            else
                
                !--- Before emergence fill compartments up to shoot depth
    ...
    if(ddw_rt .gt. 0.d0 .and. nrootprof .lt. 1.d0)then
        !--- Roots are very close to planting depth (< 1 cm distance)
        !--- Allocate all biomass gained to the plantdepth compartment        
        !--- Root Dry Weight Gain by subnode [g m-2]
        ddw_rt_prof(aint(initcropdepth)) = ddw_rt * (1.e6 / 1.e4)
    
        !--- Root Lenght Density Gain by subnode [cm cm-3]
        drld_prof   = ddw_rt_prof   * srlmax  * (1.e2 / 1.e4) / sublayer_h        
    endif    
    ...
    ddw_rt_sl   = 0.d0
    n_subnodes  = 1
    !--- Allocate among soil layer compartments        
