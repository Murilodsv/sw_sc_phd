...
#--- if a map of error would be performed the number of simulations would be computed by
#--- nsim = dvar ^ npar
#where dvar is the step on parameter variation (0-1) and npar is the number of parameters
...
swap_samuca_calib = function(svalue){
  ...
  #--- Include das in all measured db
  ...
  p_swc10  = mperf(s_swc10,o_swc10,"SWC at 10cm (cm3 cm-3)", F)
  p_swc20  = mperf(s_swc20,o_swc20,"SWC at 20cm (cm3 cm-3)", F)
  p_swc30  = mperf(s_swc30,o_swc30,"SWC at 30cm (cm3 cm-3)", F)
  p_swc60  = mperf(s_swc60,o_swc60,"SWC at 60cm (cm3 cm-3)", F)
  
  p_swc    = mperf(s_swc,o_swc,"SWC (cm3 cm-3)", F)
  ...
  o_et = so_atm$et
  p_atm  = mperf(s_et,o_et,"ET (mm d-1)", F)
  ...
  p_sfm = mperf(so_sfm$tch,o_sfm$SFM, "SFM (t ha-1)",F)
  p_sdm = mperf(so_sdm$sw,o_sdm$SDM, "SDM (t ha-1)",F)
  p_lai = mperf(so_lai$lai,o_lai$LAIGD, "LAI (m2 m-2)",F)
  p_til = mperf(so_til$till,o_til$T.AD, "Tiller (till m-2)",F)
  p_pol = mperf(so_pol$pol,o_pol$SU.FMD, "POL (%)",F)
  p_dgl = mperf(so_dgl$devgl,o_dgl$N.GL, "N° dev GL per stalk",F)
  p_sth = mperf(so_sth$h,o_sth$SHTD, "Height (m)",F)
