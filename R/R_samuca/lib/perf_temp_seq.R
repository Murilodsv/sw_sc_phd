...
perf.temp.seq = function(obs.f,
  ...
  s.date    = as.POSIXct(paste0("01/01/",as.numeric(sim.ctl$rep[sim.ctl$find == "<s.ini.yr>"])), format="%m/%d/%Y", tz = "GMT") + (as.numeric(sim.ctl$rep[sim.ctl$find == "<s.ini.doy>"]) - 1) * 24 * 60 * 60
  ...
  for(o in 1:n.obs.var){
    ...
    for(l in sim.obs.slayers.perf$samuca_soil_code_var){
      ...
      sim.df  = temp.sim[,c("year","doy","das","dap",l)]
      ...
      if(any(obs.f$meas_ID == o.id & obs.f$type == "std")){
        ...
        max.y = max(c(max(temp.sim[,sim.obs.slayers.perf$samuca_soil_code_var]),max(obs.f$value[obs.f$depth %in% sim.obs.slayers.perf$depth])))
        min.y = min(c(min(temp.sim[,sim.obs.slayers.perf$samuca_soil_code_var]),min(obs.f$value[obs.f$depth %in% sim.obs.slayers.perf$depth])))
        ylim  = c(min.y,max.y)
      }
