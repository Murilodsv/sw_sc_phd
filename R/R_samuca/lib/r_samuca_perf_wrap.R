...
r.samuca.perf.fun = function(analysis.id,
                             wd.core,
                             wd.model,
                             use.debug,
                             debug.path,
...
for(f in fields.seq){
  ...
  for(s in f.seq){
    ...
    soil.standalone.out.fn = paste0(wd.model,"\\Output\\Soil_standalone_",sim.id,".out")
    ...
    soil.sim.sa = read.soil.out.standalone(soil.standalone.out.fn, nl)
    temp.sim    = read.temp.out(temp.out.fn, nl)
    
    if(standalone.swb){
      #--- When using standalone soil water balance version
      soil.sim = soil.sim.sa
    }
