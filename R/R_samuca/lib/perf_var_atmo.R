...
perf.var.atmo = function(perf.dt,
                         meta,
                         sim.obs.idx,
                         save.plots,
                         wd.core,
  ...
  for(var in unique(perf.dt$meas_ID)){
    ...
    perf.var = merge(perf.dt[perf.dt$meas_ID == var & perf.dt$type == "obs",c("das","val","M_ID")],
                     perf.dt[perf.dt$meas_ID == var & perf.dt$type == "sim",c("das","val","M_ID")],
                     by = c("M_ID","das"))
    
    perf.var =   merge(perf.var,meta[,c("M_ID","site","Name_ID")],
                       by = "M_ID")
    
    colnames(perf.var) = c("M_ID","das","obs","sim","Name","Site")
