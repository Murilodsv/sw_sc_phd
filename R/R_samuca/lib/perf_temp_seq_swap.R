...
perf.temp.seq.swap = function(obs.f,
                         sim.ctl,
                         soil.ctl,
                         sim.obs.idx,
                         temp.sim,
  ...
  #--- Compute SAMUCA performance for simulate soil temp variables 
  ...
  soil.dp.ly$sim.nm = paste0("Ts.",soil.dp.ly$layer)
  
  #--- Read Simulated and Observed Layers Association File
  if(length(unique(meta.perf$stemp_compare_file)) == 1){
    sim.obs.slayers.idx.fn = unique(meta.perf$stemp_compare_file)
  ...
  for(o in 1:n.obs.var){
    ...
    sim.df.all = 
      get_data_dsoil(temp.sim,
                     d.nm       = 'depth',
                     dsoil.vec  = unique(soil.dp.vec),
                     exact.match= F,
                     max.dif    = 10) # max difference of 10 cm in depth to consider as comparable
    
    #--- Extract for each soil layer depth
    for(l in sim.obs.slayers.perf$samuca_soil_code_var){
      
      #--- soil
      #--- extract simulated data
      depth.l = sim.obs.slayers.perf$depth[sim.obs.slayers.perf$samuca_soil_code_var == l] * -1
      sim.df = sim.df.all[sim.df.all$depth_req == depth.l,]
      
      sim.df  = sim.df[,c("year","doy","das","dap",'temp')]
      ...
      dp.obs = sim.obs.slayers.perf$depth[sim.obs.slayers.perf$samuca_soil_code_var == l]
      ...
      if(any(obs.f$meas_ID == o.id & obs.f$type == "std")){
        ...
        max.y = max(c(max(sim.df.all$temp),max(obs.f$value[obs.f$depth %in% sim.obs.slayers.perf$depth])))
        min.y = min(c(min(sim.df.all$temp),min(obs.f$value[obs.f$depth %in% sim.obs.slayers.perf$depth])))
        ylim  = c(min.y,max.y)
      }
      ...
      if(length(obs.df$val) > 1){
        ...
        #--- Compute performance
        o.perf = mperf(sim.df$val[sim.df$das %in% obs.df$das],
                       obs.df$val, vnam = "EVAL", dchart = F)
