...
perf.plan = function(obs.f,
  ...
  #--- NON-SEQUENTIAL MODE: Only works for single season simulations
  #---------------------------------------------------------------
  #
  #--- Input Parameters:
  ...
  for(o in 1:n.obs.var){
    ...
    sim.df$type    = "sim"
    sim.df$seq     = F
    sim.df$Field_ID= meta.perf$Field_ID[meta.perf$M_ID == m.id]
    sim.df$seq_ID  = meta.perf$seq_ID[meta.perf$M_ID == m.id]
    ...
    obs.df$type    = "obs"
    obs.df$seq     = F
    obs.df$Field_ID= meta.perf$Field_ID[meta.perf$M_ID == m.id]
    obs.df$seq_ID  = meta.perf$seq_ID[meta.perf$M_ID == m.id]
    ...
    if(any(obs.f$meas_ID == o.id & obs.f$type == "std")){
      ...
      sd = obs.f$value[obs.f$meas_ID == o.id & obs.f$type == "std"]
    ...
    xlim  = c(0,max(sim.df$dap))
    
    #--- Plot (Sim + Obs) ~ DAP
    dap.sim = ggplot(obs.df,aes(x = dap, y = val)) + 
      geom_point(size = 5) + ylab(y.lab) + xlab(x.lab) +
      scale_x_continuous(limits = xlim) + scale_y_continuous(limits = ylim) +
      geom_line(data = data.frame(dap = sim.df$dap, val = sim.df$val))+
    ...
    if(length(obs.df$val) > 1){
      ...
      #--- Compute performance
      o.perf = mperf(sim.df$val[sim.df$dap %in% obs.df$dap],obs.df$val, vnam = "POL", dchart = F)
