...
perf.soil.seq = function(obs.f,
  ...
  for(o in 1:n.obs.var){
    ...
    for(l in sim.obs.slayers.perf$samuca_soil_code_var){
      ...
      sim.df  = soil.sim[,c("year","doy","das","dap",l)]
      ...
      if(any(obs.f$meas_ID == o.id & obs.f$type == "std")){
        ...
        max.y = max(c(max(soil.sim[,sim.obs.slayers.perf$samuca_soil_code_var]),max(obs.f$value[obs.f$depth %in% sim.obs.slayers.perf$depth])))
        min.y = min(c(min(soil.sim[,sim.obs.slayers.perf$samuca_soil_code_var]),min(obs.f$value[obs.f$depth %in% sim.obs.slayers.perf$depth])))
        ylim  = c(min.y,max.y)
      }
